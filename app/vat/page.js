...
export default function VatPage() {
  ...
  useEffect(() => {
    ...
    if (requested === "z" || requested === "invoice") setType(requested);
  }, []);
  useEffect(() => {
      ...
      sb.from("establishments")
        ...
        .then(({ data }) => {
          ...
          if (data?.[0]) {
            setInv((v) => ({
              ...v,
              establishment_id: v.establishment_id || data[0].id,
            }));
            setZ((v) => ({
              ...v,
              establishment_id: v.establishment_id || data[0].id,
  ...
  async function loadZ() {
    const { data } = await sb
      .from("daily_sales")
      .select(
        "id,establishment_id,business_date,lunch_covers,dinner_covers,staff_hours,staff_cost,lunch_sales_ht,dinner_sales_ht",
      )
      .order("business_date", { ascending: false })
      .limit(100);
    const sales = data || [];
    if (!sales.length) return setZRows([]);
    const { data: vat } = await sb
      .from("daily_sale_vat_lines")
      .select("daily_sale_id,vat_amount")
      .in(
        "daily_sale_id",
        sales.map((x) => x.id),
      );
    ...
    (vat || []).forEach(
      (v) =>
        (vatBy[v.daily_sale_id] =
          (vatBy[v.daily_sale_id] || 0) + Number(v.vat_amount || 0)),
    );
    setZRows(sales.map((x) => ({ ...x, vat_amount: vatBy[x.id] || 0 })));
  }
  async function editZ(row) {
    const { data: v } = await sb
      .from("daily_sale_vat_lines")
      .select("vat_rate,amount_ht")
      .eq("daily_sale_id", row.id)
      .order("vat_rate");
    setEditingZ(row.id);
    setType("z");
    setZ({
      ...
      lunch_covers:
        Number(row.lunch_covers || 0) + Number(row.dinner_covers || 0) || "",
      dinner_covers: "",
        ...
        : [
            {
              vat_rate: "0",
              amount_ht: String(
                Number(row.lunch_sales_ht || 0) +
                  Number(row.dinner_sales_ht || 0),
              ),
            },
          ],
    );
    window.scrollTo({ top: 0, behavior: "smooth" });
    setMsg("Modification du Z du " + row.business_date);
  }
  async function deleteZ(row) {
    if (
      !confirm("Supprimer définitivement le Z du " + row.business_date + " ?")
    )
      return;
    setSaving(true);
    await sb.from("daily_sale_vat_lines").delete().eq("daily_sale_id", row.id);
    const { error } = await sb.from("daily_sales").delete().eq("id", row.id);
    setSaving(false);
    if (error) return setMsg("Erreur suppression : " + error.message);
    if (editingZ === row.id) {
      setEditingZ(null);
      setLines([line()]);
    }
    ...
    if (type === "invoice") {
      ...
      if (!editingZ) {
        ...
        if (x?.length) {
          setSaving(false);
          return setMsg(
            "⚠️ Un autre Z existe déjà pour cet établissement à cette date.",
          );
        }
      }
      let p, error;
      if (editingZ) {
        ({ data: p, error } = await sb
          .from("daily_sales")
          .update({
            ...z,
        ...
        <div className="formCard" style={{ maxWidth: 850, marginTop: 18 }}>
                ...
                <div className="grid">
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setInv({ ...inv, payment_method: e.target.value })
                      }
                      style={input}
                    >
                      <option value="">Choisir…</option>
            ...
            <button disabled={saving} style={{ marginTop: 18, padding: 13 }}>
              {saving
                ...
                : editingZ
                  ...
                  : type === "invoice" && inv.document_type === "credit_note"
                    ? "Enregistrer l’avoir"
                    : "Enregistrer"}
            </button>
            {editingZ && (
          ...
          {type === "z" && (
            <div style={{ marginTop: 30 }}>
              <h2>Historique des Z de caisse</h2>
              <p>
                Vous pouvez revenir sur un Z de Villa Valleyre ou de La Maison
                du Parc pour le modifier ou le supprimer.
              </p>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {zRows.map((r) => {
                      const est = ests.find((e) => e.id === r.establishment_id);
                      return (
                        <tr key={r.id}>
                          ...
                          <td>
                            {euro(
                              Number(r.lunch_sales_ht || 0) +
                                Number(r.dinner_sales_ht || 0) +
                                Number(r.vat_amount || 0),
                            )}
                          </td>
                          <td>
                            {Number(r.lunch_covers || 0) +
                              Number(r.dinner_covers || 0)}
                          </td>
                          <td style={{ whiteSpace: "nowrap" }}>
                            <button type="button" onClick={() => editZ(r)}>
                              Modifier
                            </button>{" "}
                            <button type="button" onClick={() => deleteZ(r)}>
                              Supprimer
                            </button>
                          </td>
                        </tr>
