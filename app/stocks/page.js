...
export default function StocksPage(){
 ...
 const [est,setEst]=useState(''),[month,setMonth]=useState(monthNow()),[values,setValues]=useState({}),[msg,setMsg]=useState(''),[saving,setSaving]=useState(false)
 ...
 const current=useMemo(()=>categories.map(category=>{
  ...
  const opening=row?Number(row.opening_stock_ht||0):Number(previous?.closing_stock_ht||0)
 ...
 }),[rows,invoices,est,month,values])
 ...
 async function save(){
  ...
  setValues({});await load();setSaving(false);setMsg('✓ Stocks enregistrés. Le stock de fin sera repris automatiquement le mois suivant.')
 }
 ...
 return <main><header><div><div className="brand">MAISON ODDOS</div><h1>Stocks & inventaires</h1></div><div><a href="/"><button>← Pilotage</button></a></div></header><section>
  <div className="formCard"><div className="grid"><label><b>Établissement</b><select value={est} onChange={e=>{setEst(e.target.value);setValues({})}} style={style}>{ests.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label><b>Mois</b><input type="month" value={month} onChange={e=>{setMonth(e.target.value);setValues({})}} style={style}/></label></div></div>
  ...
  {current.map(x=><div className="formCard" key={x.category} style={{marginTop:14}}><h3>{x.category}</h3><div className="grid"><Card t="Stock début HT" v={euro(x.opening)}/><Card t="Achats HT du mois" v={euro(x.purchases)}/><label><b>Stock fin HT</b><input type="number" min="0" step="0.01" value={values[x.category]??(x.row?.closing_stock_ht??'')} placeholder="0,00" onChange={e=>setValues(v=>({...v,[x.category]:e.target.value}))} style={style}/></label><Card t="Consommation réelle" v={euro(x.consumption)}/></div><small>{x.auto?'Stock début repris automatiquement du stock fin du mois précédent.':'Premier mois disponible : le stock début peut être initialisé lors de la première saisie.'}</small></div>)}
