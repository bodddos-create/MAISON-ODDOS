...
function normalize(p,type){if(type==='z')return{restaurant:val(p.restaurant),date:val(p.date),ca:num(p.ca),covers:num(p.covers),lunch:num(p.lunch),dinner:num(p.dinner),confidence:{restaurant:conf(p.restaurant),date:conf(p.date),ca:conf(p.ca),covers:conf(p.covers),lunch:conf(p.lunch),dinner:conf(p.dinner)}};const category=CATEGORIES.includes(val(p.category))?val(p.category):'';return{restaurant:val(p.restaurant),supplier:val(p.supplier),category,date:val(p.date),number:val(p.number),ht:num(p.ht),vat:num(p.vat),ttc:num(p.ttc),dueDate:val(p.dueDate),confidence:{restaurant:conf(p.restaurant),supplier:conf(p.supplier),category:category?conf(p.category):0,date:conf(p.date),number:conf(p.number),ht:conf(p.ht),vat:conf(p.vat),ttc:conf(p.ttc),dueDate:conf(p.dueDate)}}}
