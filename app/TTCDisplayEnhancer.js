 ...
 useEffect(()=>{
  ...
  const run=async()=>{
   ...
   const apply=()=>{
    ...
    const caHt=filtered.reduce((a,x)=>a+Number(x.lunch_sales_ht||0)+Number(x.dinner_sales_ht||0),0)
    ...
    const greenCard=main.querySelector('[data-daily-fixed-personnel="true"]');if(greenCard){let box=greenCard.querySelector('[data-daily-fixed-personnel-rate]');if(!box){box=document.createElement('div');box.dataset.dailyFixedPersonnelRate='true';box.style.cssText='margin-top:8px;font-size:13px;font-weight:800;padding:5px 9px;border-radius:8px;display:inline-block;background:rgba(255,255,255,.18);color:#fff';greenCard.appendChild(box)}const dailyCaHt=estIds.reduce((sum,id)=>{const estSales=filtered.filter(x=>x.establishment_id===id),estCa=estSales.reduce((a,x)=>a+Number(x.lunch_sales_ht||0)+Number(x.dinner_sales_ht||0),0),salesDays=new Set(estSales.filter(x=>Number(x.lunch_sales_ht||0)+Number(x.dinner_sales_ht||0)>0).map(x=>x.business_date)).size;return sum+(salesDays?estCa/salesDays:0)},0),dailyFixedPersonnelHt=estIds.reduce((sum,id)=>sum+(payrollByEst[id]||0)/realOpenDays(id)+monthFixes.filter(x=>x.establishment_id===id).reduce((a,x)=>a+Number(x.amount||0),0)/realOpenDays(id),0),rate=dailyCaHt?dailyFixedPersonnelHt/dailyCaHt*100:0;box.textContent=rate.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})+' % du CA HT / jour'}
    ...
    const dailyCostRoot=main.querySelector('[data-daily-cost="true"]'),dailyCostCard=dailyCostRoot?.querySelector('strong');if(dailyCostCard)dailyCostCard.textContent=money(dailyCost)
    if(dailyCostRoot){let box=dailyCostRoot.querySelector('[data-daily-cost-rate]');if(!box){box=document.createElement('div');box.dataset.dailyCostRate='true';box.style.cssText='margin-top:8px;font-size:13px;font-weight:800;padding:5px 9px;border-radius:8px;display:inline-block;background:rgba(255,255,255,.18);color:#fff';dailyCostRoot.appendChild(box)}const dailyCaHt=estIds.reduce((sum,id)=>{const estSales=filtered.filter(x=>x.establishment_id===id),estCa=estSales.reduce((a,x)=>a+Number(x.lunch_sales_ht||0)+Number(x.dinner_sales_ht||0),0),salesDays=new Set(estSales.filter(x=>Number(x.lunch_sales_ht||0)+Number(x.dinner_sales_ht||0)>0).map(x=>x.business_date)).size;return sum+(salesDays?estCa/salesDays:0)},0),dailyCostHt=estIds.reduce((sum,id)=>{const open=realOpenDays(id),p=(payrollByEst[id]||0)/open,f=monthFixes.filter(x=>x.establishment_id===id).reduce((a,x)=>a+Number(x.amount||0),0)/open,a=monthInv.filter(x=>x.establishment_id===id).reduce((s,x)=>s+Number(x.amount_ht||0),0)/open;return sum+p+f+a},0),rate=dailyCaHt?dailyCostHt/dailyCaHt*100:0;box.textContent=rate.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})+' % du CA HT / jour'}
