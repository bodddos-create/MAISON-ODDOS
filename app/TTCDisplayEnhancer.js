 ...
 useEffect(()=>{
  ...
  const run=async()=>{
   ...
   const apply=()=>{
    ...
    const fixedHt=monthFixes.reduce((a,x)=>a+Number(x.amount||0),0),openCount=estIds.reduce((a,id)=>a+realOpenDays(id),0)||1;addRate('Charges fixes','fixed',fixedTotal,fixedHt/openCount);addRate('Personnel','personnel',payroll,payroll/openCount)
