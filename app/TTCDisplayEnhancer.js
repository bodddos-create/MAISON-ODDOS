...
const isOpen = (est, date) => {
  const d = date.getDay();
  if (est === VILLA) return d >= 2 && d <= 6;
  if (est === PARC) return d >= 1 && d <= 6;
  return d >= 1 && d <= 6;
};
const openDaysInMonth = (est, ym) => {
  const [y, m] = ym.split("-").map(Number),
    days = new Date(y, m, 0).getDate();
  let n = 0;
  for (let i = 1; i <= days; i++) if (isOpen(est, new Date(y, m - 1, i))) n++;
  return n || 1;
};
    ...
    const run = async () => {
      ...
      const [
        ...
        sb
          ...
          .select(
            "establishment_id,invoice_date,amount_ht,vat_amount,document_type",
          ),
        sb
          .from("employees")
          .select("establishment_id,monthly_loaded_cost,active")
          .eq("active", true),
        sb
          .from("fixed_charges")
          .select(
        ...
        sb
          .from("opening_days")
          .select("establishment_id,business_date,is_open"),
      ]);
      const vatBySale = {};
      (vat || []).forEach(
        (v) =>
          (vatBySale[v.daily_sale_id] =
            (vatBySale[v.daily_sale_id] || 0) + Number(v.amount_ttc || 0)),
      );
      const rows = (sales || []).map((s) => {
        const ht =
          Number(s.lunch_sales_ht || 0) + Number(s.dinner_sales_ht || 0);
        return {
          ...s,
          ttc: Object.prototype.hasOwnProperty.call(vatBySale, s.id)
            ? vatBySale[s.id]
            : ht,
          covers: Number(s.lunch_covers || 0) + Number(s.dinner_covers || 0),
        };
      });
      const apply = () => {
        if (stopped) return;
        ...
        if (!main) return;
        const estSelect = [...main.querySelectorAll("select")].find((x) =>
        ...
        const emps =
          est === "all"
            ? employees || []
            : (employees || []).filter((x) => x.establishment_id === est);
        const fixes =
          est === "all"
            ? fixedCharges || []
            : (fixedCharges || []).filter((x) => x.establishment_id === est);
        const ca = filtered.reduce((a, x) => a + x.ttc, 0);
        const caHt = filtered.reduce(
          (a, x) =>
        ...
        const days = new Set(
          filtered.filter((x) => x.ttc > 0).map((x) => x.business_date),
        ).size;
        const avg = days ? ca / days : 0;
        const achats = inv.reduce(
          (a, x) =>
            a +
          ...
          if (custom.length) {
            const [y, m] = ym.split("-").map(Number),
              ...
              by = Object.fromEntries(
                custom.map((x) => [x.business_date, x.is_open]),
              );
            let total = 0;
            for (let d = 1; d <= n; d++) {
              const ds = ym + "-" + String(d).padStart(2, "0");
              if (ds in by ? by[ds] : isOpen(id, new Date(y, m - 1, d)))
                total++;
            }
            return total || 1;
          }
          return openDaysInMonth(id, ym);
        };
        const payroll = emps.reduce(
          (a, x) => a + Number(x.monthly_loaded_cost || 0),
          0,
        );
        const payrollByEst = {};
        emps.forEach(
          (e) =>
            (payrollByEst[e.establishment_id] =
              (payrollByEst[e.establishment_id] || 0) +
              Number(e.monthly_loaded_cost || 0)),
        );
        const personnelDay = Object.entries(payrollByEst).reduce(
          (sum, [id, total]) => sum + total / realOpenDays(id),
          0,
        );
        const chargeApplies = (x, m) => {
          const start = String(x.month || "").slice(0, 7),
            end = String(x.end_month || "").slice(0, 7);
          return x.recurring ? start <= m && (!end || end >= m) : start === m;
        };
        const monthFixes = fixes.filter((x) => chargeApplies(x, ym));
        const fixedTotal = monthFixes.reduce(
          (a, x) =>
        ...
        const dailyPurchases = estIds.reduce(
          (sum, id) =>
            sum +
            monthInv
        ...
        const dailyFixed = estIds.reduce(
          (sum, id) =>
            sum +
            monthFix
              .filter((x) => x.establishment_id === id)
              .reduce(
                (a, x) =>
                  a +
                  Number(x.amount_ttc != null ? x.amount_ttc : x.amount || 0),
                0,
              ) /
              realOpenDays(id),
          0,
        ...
        const setCard = (title, val) => {
          [...main.querySelectorAll(".card")].forEach((c) => {
            const s = c.querySelector("span"),
              b = c.querySelector("strong");
            if (s?.textContent === title && b) b.textContent = val;
          });
        };
        const hero = [...main.querySelectorAll(".hero small")].find((x) =>
          x.textContent?.includes("Chiffre d’affaires"),
        );
        if (hero) {
          hero.textContent = "Chiffre d’affaires TTC enregistré";
        ...
        setCard("CA estimé sur 22 jours TTC", money(avg * 22));
        setCard("CA moyen / jour", money(avg));
        ...
        const purchaseCard = [...main.querySelectorAll(".card")].find(
          (c) => c.querySelector("span")?.textContent?.trim() === "Achats TTC",
        );
        if (purchaseCard) {
          let pct = purchaseCard.querySelector("[data-purchase-rate]");
          if (!pct) {
            pct = document.createElement("div");
            pct.dataset.purchaseRate = "true";
            pct.style.cssText =
        ...
        const addRate = (title, key, ttcValue, htValue) => {
          const card = [...main.querySelectorAll(".card")].find(
            (c) => c.querySelector("span")?.textContent?.trim() === title,
          );
          if (!card) return;
          let box = card.querySelector('[data-rate="' + key + '"]');
          if (!box) {
            box = document.createElement("div");
            box.dataset.rate = key;
            box.style.cssText =
              "margin-top:8px;font-size:13px;font-weight:800;padding:5px 9px;border-radius:8px;display:inline-block;color:#2f6b3a;background:#e8f3e8";
            card.appendChild(box);
          }
          const htPct = caHt ? (htValue / caHt) * 100 : 0;
          box.textContent =
            htPct.toLocaleString("fr-FR", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            }) + " % du CA HT";
        };
        ...
        if (greenCard) {
          let box = greenCard.querySelector(
            "[data-daily-fixed-personnel-rate]",
          );
          if (!box) {
            box = document.createElement("div");
            box.dataset.dailyFixedPersonnelRate = "true";
            box.style.cssText =
              "margin-top:8px;font-size:13px;font-weight:800;padding:5px 9px;border-radius:8px;display:inline-block;background:rgba(255,255,255,.18);color:#fff";
            greenCard.appendChild(box);
          }
          const dailyCaHt = estIds.reduce((sum, id) => {
              const estSales = filtered.filter(
              ...
              return sum + (salesDays ? estCa / salesDays : 0);
            }, 0),
            dailyFixedPersonnelHt = estIds.reduce(
              (sum, id) =>
                sum +
                (payrollByEst[id] || 0) / realOpenDays(id) +
                monthFixes
                  .filter((x) => x.establishment_id === id)
            ...
            rate = dailyCaHt ? (dailyFixedPersonnelHt / dailyCaHt) * 100 : 0;
          box.textContent =
        ...
        if (fixedPersonnelCard)
          fixedPersonnelCard.textContent = money(dailyFixedPersonnel);
        const dailyCostRoot = main.querySelector('[data-daily-cost="true"]'),
          dailyCostCard = dailyCostRoot?.querySelector("strong");
        if (dailyCostCard) dailyCostCard.textContent = money(dailyCost);
        if (dailyCostRoot) {
          let box = dailyCostRoot.querySelector("[data-daily-cost-rate]");
          if (!box) {
            box = document.createElement("div");
            box.dataset.dailyCostRate = "true";
            box.style.cssText =
              "margin-top:8px;font-size:13px;font-weight:800;padding:5px 9px;border-radius:8px;display:inline-block;background:rgba(255,255,255,.18);color:#fff";
            dailyCostRoot.appendChild(box);
          }
          const dailyCaHt = estIds.reduce((sum, id) => {
              const estSales = filtered.filter(
                  (x) => x.establishment_id === id,
            ...
            dailyCostHt = estIds.reduce((sum, id) => {
              const open = realOpenDays(id),
                p = (payrollByEst[id] || 0) / open,
                f =
                  monthFixes
                    .filter((x) => x.establishment_id === id)
                    .reduce((a, x) => a + Number(x.amount || 0), 0) / open,
                a =
                  monthInv
                    .filter((x) => x.establishment_id === id)
                    .reduce(
            ...
            rate = dailyCaHt ? (dailyCostHt / dailyCaHt) * 100 : 0;
          box.textContent =
            rate.toLocaleString("fr-FR", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
        ...
        const avgCard = [...main.querySelectorAll(".card")].find(
          (c) =>
            c.querySelector("span")?.textContent?.trim() ===
            "CA moyen / jour TTC",
        );
        if (avgCard) {
          let delta = avgCard.querySelector("[data-daily-balance]");
          if (!delta) {
            delta = document.createElement("div");
            delta.dataset.dailyBalance = "true";
            delta.style.cssText =
              "margin-top:8px;font-size:13px;font-weight:800;padding:5px 9px;border-radius:8px;display:inline-block";
            avgCard.appendChild(delta);
          }
          const balance = avg - dailyCost;
          delta.textContent =
            (balance >= 0 ? "+" : "") + money(balance) + " vs coût journalier";
          delta.style.color = balance >= 0 ? "#2f6b3a" : "#b42318";
          delta.style.background = balance >= 0 ? "#e8f3e8" : "#fde8e7";
        }
        [...main.querySelectorAll(".card span")].forEach((s) => {
          if (s.textContent === "Personnel moyen / jour (22j)")
            s.textContent = "Personnel moyen / jour d’ouverture";
          if (
            [
              "CA estimé sur 22 jours",
              "CA moyen / jour",
              "Ticket moyen",
              "Achats",
            ].includes(s.textContent) &&
            !s.textContent.includes("TTC")
          )
            s.textContent += " TTC";
        });
        ...
        [...main.querySelectorAll("small")].forEach((s) => {
          if (s.textContent?.startsWith("Charges = achats + personnel"))
            s.textContent =
              "Graphique de gestion conservé en HT : achats + personnel + charges fixes. L’écart représente CA HT − charges.";
        });
      };
      apply();
